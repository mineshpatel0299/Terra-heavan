...
const beforeAfterStyles = `
  ...
  [data-glow]::before {
    background-image: radial-gradient(
      ...
      calc(var(--x, 0) * 1px)
      calc(var(--y, 0) * 1px),
      hsl(var(--hue, 38) calc(var(--saturation, 80) * 1%) calc(var(--lightness, 55) * 1%) / var(--border-spot-opacity, 1)), transparent 100%
    );
    filter: brightness(2);
  }
...
const GlowCard: React.FC<GlowCardProps> = ({
  ...
  const inlineStyles: React.CSSProperties & Record<string, string | number> = {
    ...
    backgroundImage: `radial-gradient(
      ...
      calc(var(--x, 0) * 1px)
      calc(var(--y, 0) * 1px),
      hsl(var(--hue, 38) 70% 60% / 0.08), transparent
    )`,
